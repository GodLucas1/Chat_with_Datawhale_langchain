...
LLM_MODEL_DICT = {
    ...
    "zhipuai": ["chatglm_pro", "chatglm_std", "chatglm_lite", "glm-4-flash"]
}
...
def format_chat_prompt(model: str, message, chat_history):
    """
    该函数用于格式化聊天 prompt。
    ...
    if model in ["chatglm_pro", "chatglm_std", "chatglm_lite", "glm-4-flash"]:
        messages = []
        ...
        for turn in chat_history:
            ...
            user_prompt["content"] = user_message
            messages.append(user_prompt)
            bot_prompt = {}
            bot_prompt["role"] = "assistant"
            bot_prompt["content"] = bot_message
            messages.append(bot_prompt)
...
def respond(message, chat_history, llm, history_len=3, temperature=0.1, max_tokens=4095):
    """
    该函数用于生成机器人的回复。
    ...
    try:
        ...
        formatted_prompt = format_chat_prompt(llm, message, chat_history)
